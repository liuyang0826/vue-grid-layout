...
export function withLayoutItem(layout: Layout, item: LayoutItem, cb: (layoutItem: LayoutItem) => LayoutItem) {
...
function resolveCompactionCollision(layout: Layout, item: LayoutItem, moveToCoord: number, axis: 'x' | 'y') {
  ...
  for (let i = itemIndex + 1; i < layout.length; i += 1) {
    ...
    if (collides(item, otherItem)) {
      resolveCompactionCollision(layout, otherItem, moveToCoord + (item[sizeProp as keyof LayoutItem] as number), axis)
    }
  }
 ...
 */
export function getLayoutItem(layout: Layout, id: number): LayoutItem | undefined {
  for (let i = 0, len = layout.length; i < len; i += 1) {
